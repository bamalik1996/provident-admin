        ...
        <div class="col">
            <div class="card">
                <div class="card-body">
                    ...
                    <hr>
                    <form class="row g-3" action="{{ route('roles.update', ['role' => $role->id]) }}"
                        method="POST">
                        @csrf
                        @method('put')
                        <div class="col-md-6">
                            <label for="display_name" class="form-label">Role Name*</label>
                            <input type="name" name="display_name" value="{{ old('display_name', $role->display_name) }}"
                        ...
                        <div class="col-md-12">
                            <div class="row" data-masonry='{"percentPosition": true }'>
                                @foreach (config('sideBarPermissions') as $key => $value)
                                    <div class="col-sm-4 col-md-3 col-lg-3 col-xxl-2 mt-3">
                                        <ul class="list-group select-role-checkbox">
                                            ...
                                            @foreach ($value as $item)
                                                <li
                                                    ...
                                                    <span>
                                                        {{ $item }}
                                                        {{-- <br />{{ Str::slug($key . ' ' . $item, '.') }} --}}
                                                    </span>
                                                    <span class="badge rounded-pill">
                                                        <input @if (old('permissions', $permissions) && in_array($key . ' ' . $item, old('permissions', $permissions))) checked @endif
                                                            type="checkbox" class="select-inner-role-checkbox"
